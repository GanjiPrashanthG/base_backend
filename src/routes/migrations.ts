...
export function createMigrationsRouter(
  mongo: AppMongo,
  env: Pick<Env, "NODE_ENV" | "ADMIN_API_KEY" | "MONGO_MIGRATE_CHANGELOG_NAME">,
) {
  const router = Router();
  ...
  router.get(
    ...
    asyncHandler(async (_req, res) => {
      const migrations = await readMigrationChangelog(mongo.db, env.MONGO_MIGRATE_CHANGELOG_NAME);
      res.json({
